...
@section('content')
    @include('admin.layouts.components.header', [
        'title' => __('messages.list', [
            'name' => trans_choice('content.event', 2),
        ]),
        'breadcrumbs' => Breadcrumbs::render('admin.events.index'),
        'create_btn' => [
            'status' => true,
...
@push('scripts')
    <script>
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            var show_url = `{{ url('/') }}/admin/events/` + row['id'] +
                                `?tab=details`;
                            var show_data = actionShowButton(show_url);
                            var edit_url = `{{ url('/') }}/admin/events/` + row['id'] +
                                `/edit/?tab=edit`;
        ...
        $(document).on('click', '.clsstatus', function() {
            ...
            var url = `{{ url('/') }}/admin/events/status/` + id + `/` + status;
