...
@section('content')
    ...
    @php
        $add_data = view('admin.category.create');
    @endphp
    ...
    @include('admin.layouts.components.manager_modal.edit_form', [
        ...
        'edit_form_blade' => 'admin.category.edit',
    ])
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.clsShowModal', function() {
            var id = $(this).attr('data-id');
            var url = `{{ url('/') }}/admin/categories/` + id + `?tab=details`;
    ...
    <script>
        $(document).on('submit', '#CategoryFormUpdate', function(e) {
            e.preventDefault();
            var id = $('#modal_id').val();
            let formData = new FormData(this);
            $.ajax({
