...
@section('content')
    @include('admin.layouts.components.header', [
        'title' => __('messages.list', [
            'name' => trans_choice('content.impact_story', 2),
        ]),
        'breadcrumbs' => Breadcrumbs::render('admin.impact-stories.index'),
        'create_btn' => [
            'status' => true,
            'route' => route('admin.impact-stories.create'),
    ...
    <script>
        ...
        $(document).ready(function() {
            oTable = $('#kt_table_1').DataTable({
                ...
                ajax: {
                    "url": "{{ route('admin.impact-stories.index') }}",
                    data: function(d) {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            var show_url = `{{ url('/') }}/admin/impact-stories/` + row['id'] +
                                `?tab=details`;
                            var show_data = actionShowButton(show_url);
                            var edit_url = `{{ url('/') }}/admin/impact-stories/` + row['id'] +
                                `/edit/?tab=edit`;
        ...
        $(document).on('click', '.clsdelete', function() {
            ...
            var url = `{{ url('/') }}/admin/impact-stories/` + id;
        ...
        $(document).on('click', '.clsstatus', function() {
            ...
            var url = `{{ url('/') }}/admin/impact-stories/status/` + id + `/` + status;
