<div class="top-bar">
                ...
                <div class="mb-5">
                    <a href="" class="flex items-center mt-2">
                        <div class="w-8 h-8 image-fit">
                            <img alt="Starrae" class="rounded-full"
                                src="dist/images/profile-13.jpg">
                        </div>
                        <div class="ml-3">Angelina Jolie</div>
                        <div class="ml-auto w-48 truncate text-gray-600 text-xs text-right">angelinajolie@left4code.com
                        </div>
                    </a>
                    <a href="" class="flex items-center mt-2">
                        <div class="w-8 h-8 image-fit">
    ...
    <div class="intro-x dropdown w-8 h-8 relative">
        <div class="dropdown-toggle w-8 h-8 rounded-full overflow-hidden shadow-lg image-fit zoom-in">
            <img alt="Starrae"
                src="{{ isset($auth_user->image) ? $auth_user->image : blankUserUrl() }}">
        </div>
        <div class="dropdown-box mt-10 absolute w-56 top-0 right-0 z-20">
            <div class="dropdown-box__content box bg-theme-38 text-white">
                <div class="p-3 border-b border-theme-40">
                    <div class="font-medium">{{ isset($auth_user->name) ? $auth_user->name : 'Guest' }}</div>
                    <div class="text-xs text-theme-41">{{ isset($auth_user->email) ? $auth_user->email : '' }}</div>
                </div>
                <div class="">
                    <a href="{{ route('admin.profile', 'tab=details') }}"
                        class="flex items-center block p-2 transition duration-300 ease-in-out hover:bg-theme-1 rounded-md">
                        <i data-feather="user" class="w-4 h-4 mr-2"></i> Profile </a>
                </div>
                <div class=" border-t border-theme-40">
                    <form id="logout-form" action="{{ route('admin.logout') }}" method="GET">
                        ...
                        <a class="flex items-center block p-2 transition duration-300 ease-in-out hover:bg-theme-1 rounded-md"
                            ...
                            {{ trans_choice('messages.sign_out', 1) }} </a>
                    </form>
                </div>
            </div>
        </div>
