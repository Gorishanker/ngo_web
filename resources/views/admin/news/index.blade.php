...
@section('content')
    @include('admin.layouts.components.header', [
        'title' => __('messages.list', [
            'name' => trans_choice('content.news', 2),
        ]),
        'breadcrumbs' => Breadcrumbs::render('admin.news.index'),
        'create_btn' => [
            'status' => true,
...
@push('scripts')
    <script>
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            var show_url = `{{ url('/') }}/admin/news/` + row['id'] +
                                `?tab=details`;
                            var show_data = actionShowButton(show_url);
                            var edit_url = `{{ url('/') }}/admin/news/` + row['id'] +
                                `/edit/?tab=edit`;
        ...
        $(document).on('click', '.clsstatus', function() {
            ...
            var url = `{{ url('/') }}/admin/news/status/` + id + `/` + status;
