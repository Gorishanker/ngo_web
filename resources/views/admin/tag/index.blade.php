...
@section('content')
    @include('admin.layouts.components.header', [
        'title' => __('messages.list', [
            'name' => trans_choice('content.tag', 2),
        ]),
        'breadcrumbs' => Breadcrumbs::render('admin.tags.index'),
        'create_modal' => [
            'status' => true,
            'id' => 'addNewModal',
            'name' => 'Tag',
    ...
    @php
        $add_data = view('admin.tag.create');
    @endphp
    ...
    @include('admin.layouts.components.manager_modal.edit_form', [
        ...
        'edit_form_blade' => 'admin.tag.edit',
    ])
...
@push('scripts')
            ...
            oTable = $('#kt_table_1').DataTable({
                ...
                columns: [{
...

                    {
                        ...
                        render: function(data, type, row, meta) {
                            if (data) {
                                return `<div class="text-dark text-hover-primary fs-6">${data}</div>`;
                            } else {
    ...
    <script>
        $(document).on('submit', '#TagFormUpdate', function(e) {
            e.preventDefault();
            var id = $('#modal_id').val();
            let formData = new FormData(this);
            $.ajax({
